...
def run(
    ...
    try:
        ...
        retrieved_products = rag.retrieve(safe_transcript, price_constraints=price_constraints)
    ...
    logger.info("PIPELINE | RAG returned %d products (price_filter=%s)", len(retrieved_products), price_constraints or "none")
    ...
    for i, p in enumerate(retrieved_products[:5]):
        print(f"   [{i+1}] id={p.get('id')} | {p.get('name','?')[:50]} | ₹{p.get('price','?')}")
    ...
    if not retrieved_products and llm_response.get("intent") == "product_search":
        ...
        llm_response["response_text"] = "I'm sorry, I couldn't find any products matching your request in our current inventory."
    ...
    try:
        ...
        orig_actions = [a.get("action") for a in llm_response.get("ui_actions", []) if isinstance(a, dict)]
        ...
        if "SHOW_PRODUCTS" in orig_actions and "SHOW_PRODUCTS" not in val_actions:
            logger.warning("PIPELINE | Detected LLM hallucination: SHOW_PRODUCTS was completely blocked. Overriding response.")
            validated["intent"] = "out_of_stock"
            validated["ui_actions"] = []
            validated["response_text"] = "I'm sorry, I couldn't find any products matching your request in our current inventory."
    ...
    print(f"   Intent: {validated.get('intent','?')} | Confidence: {validated.get('confidence','?')}")
    ...
    print(f"🔊 TTS: {'Generated audio' if audio_b64 else 'No audio (failed or skipped)'}")
...
def run_stream(
    ...
    if text_input:
        ...
        yield json.dumps({"event": "error", "data": {"error": "No audio or text input provided."}}) + "\n\n"
        return
        
    yield json.dumps({"event": "transcript", "data": {"transcript": transcript}}) + "\n\n"
    ...
    try:
        ...
        if not skip_tts:
            try:
                ...
                pass
        yield json.dumps({"event": "audio", "data": {"response_text": msg, "audio_b64": audio_b64}}) + "\n\n"
        return
    ...
    yield json.dumps({"event": "actions", "data": {"ui_actions": validated.get("ui_actions", [])}}) + "\n\n"
    ...
    # Yield audio
    yield json.dumps({"event": "audio", "data": {
        "response_text": validated["response_text"],
        "audio_b64": audio_b64
    }}) + "\n\n"
