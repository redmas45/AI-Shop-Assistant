...
from api.models import (
    ...
    CartItemResponse, AddToCartRequest, CheckoutRequest
)
...
async def shop(
    audio: Optional[UploadFile] = File(None,  description="Audio file (WAV, MP3, WebM, OGG)"),
    text:  Optional[str]        = Form(None,  description="Text input (for testing without audio)"),
    skip_tts: bool              = Form(False, description="Skip TTS to reduce latency (testing)"),
    conversation_history: Optional[str] = Form(None, description="JSON array of prior conversation turns"),
):
    """
...
async def api_checkout_cart(req: CheckoutRequest):
    ...
    try:
        ...
        if req.payment_method and req.payment_method != "N/A" and req.payment_method != "Not Provided":
        ...
        # Custom styles
        title_style = ParagraphStyle('Title', parent=styles['Heading1'], fontSize=24, spaceAfter=12, textColor=colors.HexColor('#2c3e50'))
        subtitle_style = ParagraphStyle('Subtitle', parent=styles['Normal'], fontSize=12, spaceAfter=20, textColor=colors.HexColor('#7f8c8d'))
        
        # Header
        elements.append(Paragraph("<b>AI-KART INVOICE</b>", title_style))
        elements.append(Paragraph("Thank you for your futuristic purchase!", subtitle_style))
        elements.append(Spacer(1, 12))
        
        # Customer Info
        elements.append(Paragraph(f"<b>Delivery Address:</b> {final_address}", styles["Normal"]))
        elements.append(Paragraph(f"<b>Payment Method:</b> {final_payment}", styles["Normal"]))
        ...
        elements.append(Paragraph("<i>Your intelligent items will be dispatched shortly. Have a nice day!</i>", styles["Normal"]))
